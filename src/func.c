...
int compare(const void *a, const void *b) {
    if (((index_record_t *) a)->time_mark < ((index_record_t *) b)->time_mark)
        return -1;
    else if (((index_record_t *) a)->time_mark > ((index_record_t *) b)->time_mark)
        return 1;
    else
        return 0;
}

void sort_block(index_record_t *current, int block_size, int (*compare)(const void *, const void *)) {
    qsort(current, block_size, sizeof(index_record_t), compare);
}

void merge_blocks(index_record_t *current, int mergeStep, int block_size, int bufSize) {
    index_record_t *left = (index_record_t *) malloc(bufSize * sizeof(index_record_t));
    memcpy(left, current, (mergeStep / 2) * block_size * sizeof(index_record_t));

    index_record_t *right = (index_record_t *) malloc(bufSize * sizeof(index_record_t));
    memcpy(right, current + (mergeStep / 2) * block_size, (mergeStep / 2) * block_size * sizeof(index_record_t));
...
void *sort_file_in_memory(void *data) {
    ...
    cur = (index_record_t *)(file_start + 1);
